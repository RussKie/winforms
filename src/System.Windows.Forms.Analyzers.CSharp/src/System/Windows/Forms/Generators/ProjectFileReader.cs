    ...
    internal static class ProjectFileReader
    {
        public static ApplicationConfig? ReadApplicationConfig(GeneratorExecutionContext context)
        {
            ...
            ApplicationConfig projectConfig = new()
            {
                ...
                FontFamily = context.AnalyzerConfigOptions.GetMSBuildProperty(ApplicationConfig.PropertyNameCSharp.FontFamily, /* we want null */null!),
                FontSize = fontSize,
        ...
        {
            value = defaultValue;
            string rawValue = context.AnalyzerConfigOptions.GetMSBuildProperty(propertyName);
        ...
        {
            fontSize = null;
            string rawValue = context.AnalyzerConfigOptions.GetMSBuildProperty(ApplicationConfig.PropertyNameCSharp.FontSize);
            if (!string.IsNullOrEmpty(rawValue))
            {
                if (string.IsNullOrWhiteSpace(rawValue) ||
        ...
        private static bool TryReadHighDpiMode(GeneratorExecutionContext context, out HighDpiMode highDpiMode)
        {
            ...
            string rawValue = context.AnalyzerConfigOptions.GetMSBuildProperty(ApplicationConfig.PropertyNameCSharp.HighDpiMode);
            if (!string.IsNullOrEmpty(rawValue))
            {
                if (string.IsNullOrWhiteSpace(rawValue) ||

...
namespace System.Windows.Forms
{
    public partial class AccessibleObject
    {
        ...
        private class SystemIAccessibleWrapper : IAccessible
        {
            ...
            {
                return GetValue(() => _systemIAccessible.accNavigate(navDir, varStart));
            }

            public object accHitTest(int xLeft, int yTop)
            {
                return GetValue(() => _systemIAccessible.accHitTest(xLeft, yTop));
            }
            ...
            public object accParent => GetValue(() => _systemIAccessible.accParent);

            public int accChildCount => GetStructValue(() => _systemIAccessible.accChildCount);
            ...
            {
                return GetValue(() => _systemIAccessible.get_accName(childID));
            }
            ...
            {
                return GetValue(() => _systemIAccessible.get_accValue(childID));
            }
            ...
            public string accDescription => GetValue(() => _systemIAccessible.accDescription);

            public string get_accDescription(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accDescription(childID));
            }

            public object accRole => GetValue(() => _systemIAccessible.accRole);

            public object get_accRole(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accRole(childID));
            }

            public object accState => GetValue(() => _systemIAccessible.accState);

            public object get_accState(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accState(childID));
            }

            public string accHelp => GetValue(() => _systemIAccessible.accHelp);

            public string get_accHelp(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accHelp(childID));
            }

            public int get_accHelpTopic(out string pszHelpFile, object childID)
            {
                string helpFile = null;
                int result = GetStructValue(() => _systemIAccessible.get_accHelpTopic(out helpFile, childID), -1);
                pszHelpFile = helpFile;
                return result;
            }

            public string accKeyboardShortcut => GetValue(() => _systemIAccessible.accKeyboardShortcut);

            public string get_accKeyboardShortcut(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accKeyboardShortcut(childID));
            }

            public object accFocus => GetValue(() => _systemIAccessible.accFocus);

            public object accSelection => GetValue(() => _systemIAccessible.accSelection);

            public string accDefaultAction => GetValue(() => _systemIAccessible.accDefaultAction);

            public string get_accDefaultAction(object childID)
            {
                return GetValue(() => _systemIAccessible.get_accDefaultAction(childID));
            }
            ...
            private TReturn GetStructValue<TReturn>(
                Func<TReturn> func,
